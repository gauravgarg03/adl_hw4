...
def extract_kart_objects(
    ...
    for i, detection in enumerate(frame_detections):
        ...
        dist_to_center = np.sqrt((center_x - image_center_x) ** 2 + (center_y - image_center_y) ** 2)

        karts.append({
            "instance_id": track_id,
            "kart_name": kart_name,
            "center": (center_x, center_y),
            "is_ego_kart": track_id == ego_kart_id,
            "distance_to_center": dist_to_center  # Store distance for comparison
        })
    ...
    if num_karts > 1:
        for kart in karts:
            ...
            qa_pairs.append({
                ...
                "answer": "left" if kart_x < ego_x else "right" if kart_x > ego_x else "same"
            })
            qa_pairs.append({
                "question": f"Is {kart_name} in front of or behind the ego car?",
                "answer": "front" if kart_y > ego_y else "behind" if kart_y < ego_y else "same"
            })
            qa_pairs.append({
